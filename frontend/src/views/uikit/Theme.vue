...
const shadeLevels = ref([50,100,200,300,400,500,600,700,800,900,950]);
// Selected shades for preview
const primaryShade = ref(layoutConfig.primaryShade || 500);
const surfaceShade = ref(layoutConfig.surfaceShade || 200);
...
watch(primaryShade, (val) => layoutConfig.primaryShade = val);
watch(surfaceShade, (val) => layoutConfig.surfaceShade = val);
</script>
...
<template>
    <div class="flex flex-col gap-6 p-6">
        ...
        <!-- Shade Level Selectors -->
        <div class="flex flex-col gap-4">
            <div class="flex items-center gap-2">
                <label class="font-semibold text-sm text-muted-color">Primary Shade</label>
                <SelectButton v-model="primaryShade" :options="shadeLevels" :allowEmpty="false" />
            </div>
            <div class="flex items-center gap-2">
                <label class="font-semibold text-sm text-muted-color">Surface Shade</label>
                <SelectButton v-model="surfaceShade" :options="shadeLevels" :allowEmpty="false" />
            </div>
        </div>

        <!-- Primary Colors -->
        <div>
            <label class="block mb-2 font-semibold text-sm text-muted-color">Primary Color</label>
            <div class="flex gap-2 flex-wrap">
                <button
                    v-for="col in primaryColors"
                    :key="col.name"
                    type="button"
                    :title="col.name"
                    @click="updateColors('primary', col)"
                    :class="['border border-black w-7 h-7 rounded-full cursor-pointer', { 'ring-2 ring-primary': layoutConfig.primary === col.name }]"
                    :style="{ backgroundColor: col.name === 'noir' ? 'var(--text-color)' : col.palette[primaryShade] }"
                ></button>
            </div>
        </div>
        ...
        <!-- Surface Colors -->
        <div>
            <label class="block mb-2 font-semibold text-sm text-muted-color">Surface</label>
            <div class="flex gap-2 flex-wrap">
                <button
                    v-for="surf of surfaces"
                    :key="surf.name"
                    type="button"
                    :title="surf.name"
                    @click="updateColors('surface', surf)"
                    :class="['border border-black w-7 h-7 rounded-full cursor-pointer', { 'ring-2 ring-primary': layoutConfig.surface === surf.name }]"
                    :style="{ backgroundColor: surf.palette[surfaceShade] }"
                ></button>
            </div>
        </div>
    </div>
